    ...
    <div className="mx-auto w-[98%] mt-[20px] sm:mt-[50px] sm:w-[95%] lg:w-[80%] px-1">
      ...
      <div className="flex flex-col gap-6">
        {accounts.map((account) => {
          ...
          const hasMore = account.description.split(" ").length > 20;
          ...
          return (
            ...
            >
              ...
              <div className="flex flex-col justify-between gap-2 text-white p-4 w-full h-full">
                <div>
                  <h3 className="text-[16px] sm:text-[18px] md:text-[20px] font-bold">{account.name}</h3>
                  <h4 className="text-[14px] sm:text-[16px] md:text-[18px] text-gray-300">{account.game}</h4>

                  <p className="text-gray-400 text-[12px] sm:text-[14px] md:text-[16px] leading-relaxed">
                    {isExpanded ? account.description : getShortDescription(account.description)}
                    {!isExpanded && hasMore && "..."}
                  </p>

                  {hasMore && (
                    <button
                      onClick={() => setExpanded(prev => ({ ...prev, [account.id]: !isExpanded }))}
                      className="text-blue-400 mt-1 text-[12px] sm:text-[14px] hover:underline"
                    >
                      {isExpanded ? "Скрыть" : "Показать больше"}
                    </button>
                  )}

                  <p className="font-bold text-[14px] sm:text-[16px] md:text-[18px] text-[#f97316]">{account.price}$</p>
                </div>
